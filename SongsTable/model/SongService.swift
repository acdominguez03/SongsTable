...
class SongService{
    ...
    func getSongsFromDB(){
        for position in 0...4{
            database.collection(getBandName(bandGenre: position)).getDocuments(){
                ...
                if let error = error {
                    ...
                    self.categorySong.append(CategorySong(categoryName: self.getBandName(bandGenre: position), categorySongs: songs))
                }
            }
        }
